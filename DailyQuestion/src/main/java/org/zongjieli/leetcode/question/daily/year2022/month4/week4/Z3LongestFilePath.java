package org.zongjieli.leetcode.question.daily.year2022.month4.week4;

import java.util.LinkedList;

/**
 * 假设有一个同时存储文件和目录的文件系统
 *  dir
 *  \n\t    subdir1
 *      \n\t\t  file1.ext
 *      \n\t\t  subsubdir1
 *  \n\t    subdir2
 *      \n\t\t  subsubdir2
 *          \n\t\t\t    file2.ext
 *
 *  这里将 dir 作为根目录中的唯一目录
 *  dir 包含两个子目录 subdir1 和 subdir2
 *  subdir1 包含文件 file1.ext 和子目录 subsubdir1
 *  subdir2 包含子目录 subsubdir2,该子目录下包含文件 file2.ext
 *
 * 在文本格式中,如下所示(⟶表示制表符):
 *  dir
 *  ⟶ subdir1
 *  ⟶ ⟶ file1.ext
 *  ⟶ ⟶ subsubdir1
 *  ⟶ subdir2
 *  ⟶ ⟶ subsubdir2
 *  ⟶ ⟶ ⟶ file2.ext
 *
 * 如果是代码表示,上面的文件系统可以写为
 *  "dir\n\tsubdir1\n\t\tfile1.ext\n\t\tsubsubdir1\n\tsubdir2\n\t\tsubsubdir2\n\t\t\tfile2.ext"
 * '\n' 和 '\t' 分别是换行符和制表符
 *
 * 文件系统中的每个文件和文件夹都有一个唯一的绝对路径
 * 即必须打开才能到达文件/目录所在位置的目录顺序,所有路径用 '/' 连接
 * 上面例子中,指向 file2.ext 的 绝对路径 是 "dir/subdir2/subsubdir2/file2.ext"
 * 每个目录名由字母、数字和 / 或空格组成,每个文件名遵循 name.extension 的格式
 * 其中 name 和 extension由字母、数字和 / 或空格组成
 *
 * 给定一个以上述格式表示文件系统的字符串 input
 * 返回文件系统中指向文件的最长绝对路径的长度
 * 如果系统中没有文件,返回 0
 *
 * 1 <= input.length <= 10^4
 * input 可能包含小写或大写的英文字母,一个换行符 '\n',一个制表符 '\t',一个点 '.',一个空格 ' ' 和数字
 *
 * @author   Li.zongjie
 * @date     2022/4/20
 * @version  1.0
 */
public class Z3LongestFilePath {

    public int lengthLongestPath(String input) {
        LinkedList<String> current = new LinkedList<>();
        String[] all = input.split("\\n");
        if (all.length == 1) {
            return all[0].contains(".") ? all[0].length() : 0;
        }
        current.addLast(all[0]);
        int result = 0;
        for (int i = 1 ; i < all.length ; i++) {
            String newDir = all[i].replaceAll("\\t", "");
            int size = (all[i].length() - newDir.length());
            while (current.size() > size) {
                current.pollLast();
            }
            if (newDir.contains(".")) {
                int currentPath = newDir.length();
                for (String s : current) {
                    currentPath += (s.length() + 1);
                }
                result = Math.max(result, currentPath);
            } else {
                current.addLast(newDir);
            }
        }
        return result;
    }

    public static void main(String[] args) {
        Z3LongestFilePath test = new Z3LongestFilePath();
        // 20
//        System.out.println(test.lengthLongestPath("dir\n\tsubdir1\n\tsubdir2\n\t\tfile.ext"));
        // 5
        System.out.println(test.lengthLongestPath("a.txt"));
    }
}
